...
my_model <- multinom(Subscription_Type~., data = train)

# Make predictions
train_pred <- predict(my_model, train)
test_pred <- predict(my_model, test)

# Function to calculate metrics (same format as previous)
